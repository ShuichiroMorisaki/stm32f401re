...
/* #[<ENTRY_FUNC>]# eUart_HAL_UART_Transmit
 * name:         eUart_HAL_UART_Transmit
 * global_name:  tUart_eUart_HAL_UART_Transmit
 * oneway:       false
 * #[</ENTRY_FUNC>]# */
HAL_StatusTypeDef
eUart_HAL_UART_Transmit(CELLIDX idx, uint8_t* pData, uint16_t Size, uint32_t Timeout)
{
	CELLCB	*p_cellcb;
	if (VALID_IDX(idx)) {
		p_cellcb = GET_CELLCB(idx);
